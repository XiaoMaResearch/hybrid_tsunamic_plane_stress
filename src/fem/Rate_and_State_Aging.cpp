...
void Rate_and_State_Aging(VectorXd &M_global_vec, ArrayXi &top_surf_index, ArrayXi &bot_surf_index, VectorXd &fe_global, double dt, double dx,double dy, int nx, VectorXd &delt_u_n, VectorXd &delt_v_n,VectorXd &T_0, VectorXd &tau_s, double a, double b, double L, double V_0, double f_0, int Ndofn, double M, VectorXd &F_fault, VectorXd &T_c, VectorXd &theta_n, VectorXd &theta_dot_n)
{
...
//    
    ...
    for (int k=0; k<nx+1;k++)
    {
        double x;
        x = delt_v_n(2*k);
        NR_solve_eq10(dt, theta_pre(k), delt_v_n(2*k),M_pos(k), area, R_pos(2*k), R_neg(2*k), -T_c(2*k+1),T_0(2*k), a, b, L, V_0, f_0, x);
        V_pre(k) = x;
    }
    int iter = 1.0;
    double err = 1.0 ;
    ...
    while (iter<max_iter)
    {
        ...
        for (int k = 0; k<nx+1;k++)
        {
            double x;
            x = delt_v_n(2*k);
            NR_solve_eq10(dt, theta_cor(k), delt_v_n(2*k),M_pos(k), area, R_pos(2*k), R_neg(2*k), -T_c(2*k+1),T_0(2*k), a, b, L, V_0, f_0, x);
            V_cor(k) = x;
        }
...
//    theta_dot_n = theta_dot_new;
    ...
    for(int i=0;i<nx+1;i++)
    {
        T_c(2*i) = a*(-T_c(2*i+1))*asinh(delt_v_n(2*i)/(2*V_0)*exp((f_0+b*log(V_0*theta_n(i)/L))/a));
    }
