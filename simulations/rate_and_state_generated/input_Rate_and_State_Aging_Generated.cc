...
int main() {
   ...
   // nx_el = fault_nodes.size()-1;
    ...
    VectorXd theta_n =theta_ini*VectorXd::Ones(nx_el+1, 1);
  ...
  //      v_n(bot_surf_index(2*i))= -V_ini/2.0;
    ...
    for (int j=0;j<numt;j++)
    {
      ...
      //  Slip_Weakening(M_global_vec, top_surf_index, bot_surf_index, fe_global, dt, dx, dy, nx_el, delt_v_n, delt_u_n, T_0, tau_s, mu_s, mu_d, Dc, Ndofn, M, F_fault, T_c);
        ...
        Rate_and_State_Aging(M_global_vec, Fault_surf_index[0], Fault_surf_index[1], fe_global, dt, dx, dy, nx_el, delt_u_n, delt_v_n, T_0, tau_s, a, b, L, V_0, f_0, Ndofn, M, F_fault, T_c, theta_n, theta_dot_n);
