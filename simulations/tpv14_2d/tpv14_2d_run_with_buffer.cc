...
int main() {
   ...
   // double fault_angle  = 0/180.0*M_PI;
    ...
    for (int j=0;j<num_faults; j++)
    {
        ...
        std::cout<<"******fault_node_size="<<fault_nodes.size()<<std::endl;
        std::cout<<"******nx_faults_size="<<nx_faults.size()<<std::endl;
        fault_angle[j]=std::atan2(y2-y1,x2-x1);
      }
    std::cout<<fault_angle[0]<<"\n"<<fault_angle[1]<<std::endl;
   // fault_angle[0] = 0;
    //fault_angle[1] = -30.0/180.0*M_PI;
    //std::cout<<"hahah2=\n"<<fault_angle[0]<<fault_angle[1]<<std::endl;
//    std::cout<<"x1="<<Node(fault_nodes[0][0],0)<<std::endl;
//    std::cout<<"x2="<<Node(fault_nodes[0][nx_faults[0]-1],0)<<std::endl;
//    std::cout<<"y1="<<Node(fault_nodes[0][0],1)<<std::endl;
//    std::cout<<"y2="<<Node(fault_nodes[0][nx_faults[0]-1],1)<<std::endl;
//    std::cout<<"******fault_node_size="<<fault_nodes.size()<<std::endl;
//    std::cout<<"******nx_faults_size="<<nx_faults.size()<<std::endl;
//
//    std::cout<<"x1="<<Node(fault_nodes[2][0],0)<<std::endl;
//    std::cout<<"x2="<<Node(fault_nodes[2][nx_faults[1]-1],0)<<std::endl;
//    std::cout<<"y1="<<Node(fault_nodes[2][0],1)<<std::endl;
//    std::cout<<"y2="<<Node(fault_nodes[2][nx_faults[1]-1],1)<<std::endl;
...
//    fault_angle[1] = 0;
    ...
    std::cout<<"sigma_f[0]="<<sigma_f[0]<<"tau_f[0]="<<tau_f[0]<<std::endl;
    std::cout<<"sigma.size="<<sigma_f.size()<<"tau_f.size="<<tau_f.size()<<std::endl;

    std::cout<<"sigma_f[1]="<<sigma_f[1]<<"tau_f[1]="<<tau_f[1]<<std::endl;
   ...
   // std::cout<<x<<std::endl;
    ...
    {
        if ((x_main(i)<=(-8.0e3+1.5e3))&&(x_main(i)>=(-8.0e3-1.5e3)))
        {
            T_0[0](2*i) = 81.6e6;
        }
        if (x_main(i)>=-16.0e3&&x_main(i)<=12.0e3)
        {
            mu_s[0](i)= 0.677;
        }
        else
